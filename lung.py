...
class_counts = dataframe.groupby('CHEST PAIN').size()
print(class_counts)
...
#skew = dataframe.skew()
#print(skew)
dataframe.hist( layout=(4,4), sharex=False)
pyplot.show()
dataframe.plot(kind='density', subplots=True, layout=(4,4), sharex =False)
pyplot.show()
pyplot.figure(figsize=(10,10))
dataframe.plot(kind='box', subplots= True, layout=(4,4), sharex=False)
pyplot.show()
...
cax = ax.matshow(correlation, vmin=-1.5, vmax=1)
fig.colorbar(cax)
ticks = np.arange(0, 17, 1)
names = ['patient ID', 'GENDER', 'AGE', 'SMOKING', 'YELLOW_FINGERS', 'ANXIETY', 'PEER_PRESURE', 'CHRONIC DISEASE', 'FATIGUE', 'ALLERGY', 'WHEEZING', 'ALCOHOL CONSUMING', 'COUGHING', 'SHORTNESS OF BREATH', 'SWALLOWING DIFFICULTY', 'CHEST PAIN','LUNG_CANCER']
ax.set_xticks(ticks)
ax.set_yticks(ticks)
